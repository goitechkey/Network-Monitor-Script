...
def safe_file_component(value):
    """Make a device label safe to use in a Windows file name."""
    cleaned = "".join(
        character if character.isalnum() or character in "-_" else "-"
        for character in value.strip()
    ).strip("-_")
    return cleaned or "unnamed-device"


def history_file_for(report_date, name, ip):
    """Return one permanent, readable report file for a device and date."""
    device_name = safe_file_component(name)
    device_ip = safe_file_component(ip.replace(".", "-"))
    return HISTORY_DIR / f"ping-history-{report_date:%Y-%m-%d}-{device_name}-{device_ip}.txt"
...
def save_ping_record(category, ip, name, status, latency_ms, recorded_at):
    ...
    history_file = history_file_for(recorded_at.date(), name, ip)
...
def load_today_history(ip, name):
    """Load only today's records; previous-day files stay archived on disk."""
    history_file = history_file_for(datetime.now().date(), name, ip)
...
def ping_history():
    ...
    return render_template_string(
        ...
        records=load_today_history(ip, name),
    )
